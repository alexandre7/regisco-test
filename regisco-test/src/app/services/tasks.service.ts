...
export class TasksService {
  ...
  getTasks(): Observable<Tasks[]> {
    return this.http.get<Tasks[]>(this.tasksUrl, this.httpOptions).pipe(
  ...
  public manageTasks() {
    ...
    this.getTasks().subscribe((tasks) => {
      ...
      this.lateTasks.forEach((task) => {
        if (
          task.deadline ===
          formatDate(yesterdaydateNumber, 'yyyy-MM-dd', 'en-US')
        ) {
          task.deadline = 'Hier';
        }
      });
      ...
      this.upcomingTasks.forEach((task) => {
        if (
          task.deadline === formatDate(todaydateNumber, 'yyyy-MM-dd', 'en-US')
        ) {
  ...
  /** Envoie un message si il y a une erreur avec le fetch URL */
